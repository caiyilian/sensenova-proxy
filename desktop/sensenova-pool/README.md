...
4. 点击“同步本机客户端”后，程序会自动检测 OpenCode 和 WorkBuddy：有哪个就同步哪个，两者都有就一次全部同步，两者都没有则只提示没有需要同步的客户端，不会创建多余配置。OpenCode 的其他 provider 与注释会保留；WorkBuddy 中的 Agnes 和其他自定义模型也不会被修改。修改已有配置前会创建 `.sensenova-pool.bak` 备份。
