...
- 每次上游请求的 5 分钟超时上下文会保持到响应流完整读取或关闭，不会在刚收到响应头时提前取消；代理还会识别 HTTP 200 流中内嵌的错误以及缺失的 `[DONE]`。响应流在尚未输出任何正文前失败时可以自动换路线重试，已经向客户端输出部分内容后不会把第二次回答拼接上去，而是中止并记录 `bytesForwarded`，避免响应损坏。
