...
$outputFile = Join-Path $outputDir "AgnesProxy.exe"
...
try {
    ...
    go run github.com/akavel/rsrc@v0.10.2 -manifest "AgnesProxy.manifest" -o $resourceFile
    if ($LASTEXITCODE -ne 0) { throw "Unable to embed the Windows application manifest" }
