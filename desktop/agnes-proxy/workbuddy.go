...
type agnesWorkBuddyModelSpec struct {
	ID                string
	Name              string
	MaxInputTokens    int
	MaxOutputTokens   int
...
func agnesWorkBuddyChatURL(port int) string {
	return fmt.Sprintf("http://127.0.0.1:%d/v1/chat/completions", port)
}

func agnesWorkBuddySpecs() []agnesWorkBuddyModelSpec {
	return []agnesWorkBuddyModelSpec{
		{ID: "agnes-2.0-flash", Name: "Agnes 2.0 Flash · Resilient", MaxInputTokens: 524288, MaxOutputTokens: 65536, SupportsToolCall: true},
		{ID: "agnes-2.5-flash", Name: "Agnes 2.5 Flash · Resilient", MaxInputTokens: 524288, MaxOutputTokens: 65536, SupportsToolCall: true, SupportsImages: true},
		{ID: "agnes-3.0-flash", Name: "Agnes 3.0 Flash · Resilient", MaxInputTokens: 524288, MaxOutputTokens: 65536, SupportsToolCall: true, SupportsImages: true, SupportsReasoning: true},
	}
}

func syncAgnesWorkBuddyConfig(path, endpoint, localToken string) (WorkBuddySyncResult, error) {
	...
	specs := agnesWorkBuddySpecs()
	specByID := make(map[string]agnesWorkBuddyModelSpec, len(specs))
	...
	for _, raw := range models {
		...
		seen[id] = true
		if !looksLikeAgnesWorkBuddyModel(fields) {
			conflicts = append(conflicts, id)
			updatedModels = append(updatedModels, raw)
			continue
		}
		changed := applyAgnesWorkBuddySpec(fields, spec, endpoint, localToken)
		if changed {
	...
	for _, spec := range specs {
		...
		applyAgnesWorkBuddySpec(fields, spec, endpoint, localToken)
		encoded, marshalErr := json.Marshal(fields)
		if marshalErr != nil {
...
func looksLikeAgnesWorkBuddyModel(fields map[string]any) bool {
	vendor, _ := fields["vendor"].(string)
	name, _ := fields["name"].(string)
	endpoint, _ := fields["url"].(string)
	return strings.EqualFold(strings.TrimSpace(vendor), "Agnes") ||
		strings.Contains(strings.ToLower(name), "agnes") ||
		strings.Contains(strings.ToLower(endpoint), "agnes-ai.com") ||
		strings.Contains(endpoint, ":18788/")
}

func applyAgnesWorkBuddySpec(fields map[string]any, spec agnesWorkBuddyModelSpec, endpoint, localToken string) bool {
	changed := false
