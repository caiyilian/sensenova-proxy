...
function createGateway(options = {}) {
  ...
  const imageInspectionRetryLimit = options.imageInspectionRetryLimit ?? 2;
    ...
    while (!requestAbort.signal.aborted) {
      ...
      try {
        ...
        if (classification.category === 'image_inspection') {
          if (imageInspectionRetries >= imageInspectionRetryLimit) {
            classification.retryable = false;
            retryExhausted = true;
          } else {
            imageInspectionRetries += 1;
            retryDelayMs = imageInspectionRetryBaseDelayMs * (2 ** (imageInspectionRetries - 1));
          }
        }
