...
const DEFAULT_COOLDOWNS = Object.freeze({
  ...
  image_inspection: { baseMs: 5_000, maxMs: 15_000 },
});

const DEFAULT_EXPECTED_KEY_LENGTH = 35;

function validateSenseNovaApiKey(apiKey, expectedLength = DEFAULT_EXPECTED_KEY_LENGTH) {
  if (!apiKey.startsWith('sk-')) return 'missing_sk_prefix';
  if (!/^sk-[A-Za-z0-9._-]+$/.test(apiKey)) return 'invalid_characters';
  if (expectedLength > 0 && apiKey.length !== expectedLength) {
    return `unexpected_length_${apiKey.length}`;
  }
  return null;
}

function parseKeyFileDetailed(text, validateKey = validateSenseNovaApiKey) {
  const seen = new Map();
  const entries = [];
  const invalidLines = [];
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const validationError = validateKey ? validateKey(line) : null;
    if (validationError) {
      invalidLines.push({ lineNumber, reason: validationError });
      continue;
    }

    if (seen.has(line)) {
      duplicateLines.push({ lineNumber, duplicateOfLine: seen.get(line) });
      continue;
    }

    seen.set(line, lineNumber);
    entries.push({ apiKey: line, lineNumber });
  }

  return { entries, invalidLines, duplicateLines };
}

function parseKeyFile(text) {
  return parseKeyFileDetailed(text, null).entries.map((entry) => entry.apiKey);
}
...
function classifyFailure(status, bodyText = '') {
  ...
  if (
    status === 400 &&
    /image call nova inspection failed.*(?:rpc error|internal error)/.test(text)
  ) {
    return { category: 'image_inspection', retryable: true, scope: 'model' };
  }
...
function createJsonlLogger(logDir) {
  ...
  return (event) => {
    ...
    let details;
    if (record.event === 'keys_reloaded') {
      details = [
        record.event,
        `total=${record.accountCount}`,
        `added=${record.addedCount}`,
        `removed=${record.removedCount}`,
        `invalid=${record.invalidLineCount}`,
        `duplicates=${record.duplicateLineCount}`,
        `reason=${record.reason}`,
      ];
      if (record.addedKeyRefs?.length) details.push(`addedRefs=${record.addedKeyRefs.join(',')}`);
      if (record.removedKeyRefs?.length) details.push(`removedRefs=${record.removedKeyRefs.join(',')}`);
      if (record.invalidLines?.length) {
        details.push(`invalidLines=${record.invalidLines.map((item) => item.lineNumber).join(',')}`);
      }
    } else if (record.event === 'key_reload_failed') {
      details = [
        record.event,
        `retained=${record.retainedAccountCount}`,
        `reason=${record.reason}`,
        record.error,
      ];
    } else {
      details = [
        ...
        record.requestId,
        record.account,
        record.keyLine ? `line=${record.keyLine}` : null,
        record.keyRef ? `key=${record.keyRef}` : null,
        record.model,
        record.status,
        record.category,
      ];
...
class KeyStore {
  constructor(filePath, logger, options = {}) {
    ...
    this.logger = logger;
    const expectedKeyLength = options.expectedKeyLength ?? DEFAULT_EXPECTED_KEY_LENGTH;
    this.validateKey = options.validateKey
      || ((apiKey) => validateSenseNovaApiKey(apiKey, expectedKeyLength));
    this.watchIntervalMs = options.watchIntervalMs ?? 1_000;
    this.reloadDebounceMs = options.reloadDebounceMs ?? 250;
    this.signature = null;
    this.accounts = [];
    this.reloads = 0;
    this.invalidLines = [];
  ...
  load(reason = 'demand') {
    const fileText = fs.readFileSync(this.filePath, 'utf8');
    const signature = crypto.createHash('sha256').update(fileText).digest('hex');
    const recoveredFromError = Boolean(this.lastLoadError);
    this.lastLoadError = null;
    if (signature === this.signature) {
      if (recoveredFromError) {
        this.logger({
          event: 'key_reload_recovered',
          reason,
    ...
    const parsed = parseKeyFileDetailed(fileText, this.validateKey);
    const previousFingerprints = new Set(this.accounts.map((account) => account.fingerprint));
    const accounts = parsed.entries.map(({ apiKey, lineNumber }, index) => {
      const fingerprint = keyFingerprint(apiKey);
      return {
        id: `account-${index + 1}`,
        apiKey,
        fingerprint,
        keyRef: fingerprint.slice(0, 8),
        lineNumber,
      };
    });
    const nextFingerprints = new Set(accounts.map((account) => account.fingerprint));
    const addedKeyRefs = accounts
      .filter((account) => !previousFingerprints.has(account.fingerprint))
      .map((account) => account.keyRef);
    const removedKeyRefs = this.accounts
      .filter((account) => !nextFingerprints.has(account.fingerprint))
      .map((account) => account.keyRef);

    this.accounts = accounts;
    this.invalidLines = parsed.invalidLines;
    this.duplicateLines = parsed.duplicateLines;
    this.signature = signature;
    this.reloads += 1;
    this.logger({
      event: 'keys_reloaded',
      accountCount: this.accounts.length,
      addedCount: addedKeyRefs.length,
      removedCount: removedKeyRefs.length,
      addedKeyRefs,
      removedKeyRefs,
      invalidLineCount: this.invalidLines.length,
      invalidLines: this.invalidLines,
      duplicateLineCount: this.duplicateLines.length,
  ...
  reloadSafely(reason = 'demand') {
    try {
      return this.load(reason);
    } catch (error) {
      const safeError = redactError(error);
      if (safeError !== this.lastLoadError) {
        this.lastLoadError = safeError;
        this.logger({
          event: 'key_reload_failed',
          reason,
          error: safeError,
          retainedAccountCount: this.accounts.length,
        });
      }
      return this.accounts;
    }
  ...
  startWatching() {
    if (this.watchListener || this.watchIntervalMs <= 0) return;
    this.watchListener = () => {
      clearTimeout(this.reloadTimer);
      this.reloadTimer = setTimeout(() => {
        this.reloadTimer = null;
        this.reloadSafely('file_watch');
...
function createGateway(options = {}) {
  ...
  const imageInspectionRetryBaseDelayMs = options.imageInspectionRetryBaseDelayMs ?? 750;
  const models = options.models || [];
  const fetchImpl = options.fetchImpl || globalThis.fetch;
  const logger = options.logger || createJsonlLogger(logDir);
  const keyStore = new KeyStore(keyFilePath, logger, {
    expectedKeyLength: options.expectedKeyLength,
    validateKey: options.validateKey,
    watchIntervalMs: options.keyWatchIntervalMs,
    reloadDebounceMs: options.keyReloadDebounceMs,
  });
  ...
  function loadAccounts(reason = 'request') {
    return keyStore.reloadSafely(reason);
  }
  ...
  async function handleProxyRequest(request, response, requestUrl) {
    ...
    let nextProbeAt = 0;
    let imageInspectionRetries = 0;
    const inspectionFailed = new Set();
    let rejectInspection;

    while (!requestAbort.signal.aborted) {
      if ((completedPoolSweep && nextProbeAt > Date.now()) ||
          (attempts > 0 && Date.now() - requestStartedAt >= maxQueueMs)) {
      ...
      const candidates = accounts.filter((account) => !inspectionFailed.has(account.fingerprint));
      if (candidates.length === 0 && rejectInspection) {
        rejectInspection();
        return;
      }
      const account = pool.pick(candidates, model, attempted);
      if (!account) {
        if (attempts > 0) completedPoolSweep = true;
        attempted.clear();
        const nextReadyAt = pool.nextReadyAt(candidates, model);
        const elapsed = Date.now() - requestStartedAt;
        const remaining = maxQueueMs - elapsed;
      ...
      try {
        ...
        let retryDelayMs = 0;
        if (classification.category === 'image_inspection') {
          inspectionFailed.add(account.fingerprint);
          if (!loadAccounts().some((item) => !inspectionFailed.has(item.fingerprint))) {
            classification.retryable = false;
            retryExhausted = true;
          } else {
            imageInspectionRetries += 1;
            retryDelayMs = Math.min(5_000, imageInspectionRetryBaseDelayMs * (2 ** Math.min(imageInspectionRetries - 1, 3)));
          }
        }
        ...
        const reject = () => {
          ...
          logger({
            ...
            attempts,
            durationMs: Date.now() - requestStartedAt,
            transientRetries: classification.category === 'image_inspection'
              ? imageInspectionRetries
              : undefined,
            retryExhausted: classification.category === 'image_inspection'
              ? true
              : undefined,
            accountCount: loadAccounts().length,
            inspectionFailedAccounts: inspectionFailed.size,
          });
        ...
        if (!classification.retryable || retryExhausted) {
          pool.end(account);
          requestAbort.signal.removeEventListener('abort', abortAttempt);
          reject();
          return;
        }
        if (classification.category === 'image_inspection') rejectInspection = reject;
        ...
        logger({
          event: classification.category === 'auth' ? 'account_quarantined' : 'upstream_retry',
          requestId,
          account: account.id,
          keyLine: account.lineNumber,
          keyRef: account.keyRef,
          ...
          transientRetry: classification.category === 'image_inspection'
            ? imageInspectionRetries
            : undefined,
          retryDelayMs: classification.category === 'image_inspection'
            ? retryDelayMs
            : undefined,
          accountCount: loadAccounts().length,
          inspectionFailedAccounts: inspectionFailed.size,
        ...
        if (retryDelayMs > 0) {
          try {
            await abortableSleep(retryDelayMs, requestAbort.signal);
          } catch {
            stats.canceled += 1;
            return;
          }
  ...
  const server = http.createServer(async (request, response) => {
    ...
    if (request.method === 'GET' && requestUrl.pathname === '/health') {
      const accounts = loadAccounts('health');
      jsonResponse(response, accounts.length > 0 ? 200 : 503, {
        status: accounts.length > 0 ? 'ok' : 'no_accounts',
        accountCount: accounts.length,
        keyReloads: keyStore.reloads,
        invalidKeyLines: keyStore.invalidLines,
        duplicateKeyLines: keyStore.duplicateLines,
        keyFileWatching: Boolean(keyStore.watchListener),
        keyFileReadError: Boolean(keyStore.lastLoadError),
  ...
  return {
    ...
    listen(port, host = '127.0.0.1') {
      return new Promise((resolve, reject) => {
        ...
        keyStore.reloadSafely('startup');
        server.listen(port, host, () => {
          server.removeListener('error', reject);
          keyStore.startWatching();
          resolve(server.address());
        });
      });
    },
    close() {
      keyStore.stopWatching();
      return new Promise((resolve, reject) => {
